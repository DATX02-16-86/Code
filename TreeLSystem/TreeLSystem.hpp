...
struct TreeLSystem {
  TreeLSystem(double branchingAngle, double phyllotacticAngle, double branchLengthRatio, double branchDiameterRatio,
              int initialBranchLength, int initialBranchDiameter, string axiom, map<char, string> rules);
  
  VoxelCollection generateVoxels(int iterations);
  
protected:
  double branchingAngle, phyllotacticAngle, branchLengthRatio, branchDiameterRatio;
  int initialBranchLength, initialBranchDiameter;
  string axiom;
