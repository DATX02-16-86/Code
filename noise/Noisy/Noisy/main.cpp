...
void generateAndWrite3DTunnels(int seed, int chunkSize, int chunks, int height)
{
	// Allocate memory for points
	bool *point_z_values = (bool *)std::malloc(chunks * chunkSize * chunks * chunkSize * height * sizeof(bool *));
	...
	Terrain t = Terrain(chunks, chunkSize, seed);
	t.generateHeights();
	t.generate3DTunnels(point_z_values, height);
...
void generateAndWrite3DComposite(int seed, int chunkSize, int chunks, int height)
{
	// Allocate memory for points
	bool *point_z_values = (bool *)std::malloc(chunks * chunkSize * chunks * chunkSize * height * sizeof(bool *));
	...
	Terrain t = Terrain(chunks, chunkSize, seed);
	t.generateHeights();
	t.generate3DComposite(point_z_values, height);
...
int main() {
	...
	const int chunks = 10;
	const int height = chunkSize * 5;

	generateAndWrite3DComposite(seed, chunkSize, chunks, height);
